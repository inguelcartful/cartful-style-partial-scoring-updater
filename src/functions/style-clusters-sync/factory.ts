...
const insertDataIntoDB = async (
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    partialScoring.push({
      pk: {
        S: `cluster::${experience}::${group}::${row['cluster_id']}::${version}`,
      },
      sk: { S: '' + row['product_id'] },
      id: { S: '' + row['product_id'] },
      price: { N: '' + row['price'] },
...
const styleClustersDispatcher = async (
  ...
  const { bucket, prefix, finder, version, group } = event;
  ...
  await putStyleClusterInfo(finder, group, version, Array.from(clusters));
